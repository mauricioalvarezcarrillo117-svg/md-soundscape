...
const THEMES = {
  light: {
    ...
    input_bg: "bg-[#F0F2F5]",
    accent: "bg-[#4A90E2]",
    accent_text: "text-[#4A90E2]",
    secondary: "bg-[#E0E0E0]",
    success: "bg-[#4CAF50]",
    error: "bg-[#FF5252]",
    text_sec: "text-[#444444]",
  ...
  dark: {
    ...
    input_bg: "bg-[#2C2C2C]",
    accent: "bg-[#BB86FC]",
    accent_text: "text-[#BB86FC]",
    secondary: "bg-[#333333]",
    success: "bg-[#03DAC6]",
    error: "bg-[#CF6679]",
    text_sec: "text-[#CCCCCC]",
...
const initialCards = [
  {
    id: 1,
    question: "¿Cuál es el mecanismo de acción principal de la Metformina?",
    answer: "Disminuye la producción hepática de glucosa (gluconeogénesis) y mejora la sensibilidad a la insulina en tejidos periféricos.",
    image: null,
...
const initialQuizzes = [
  {
    id: 1,
    title: "Caso Clínico: IAMCEST",
    time_limit: 5,
    next_due: new Date().toISOString().split('T')[0],
    questions: [
      {
        text: "Paciente masculino de 55 años llega a urgencias con dolor torácico opresivo irradiado a mandíbula de 40 mins de evolución. El ECG muestra elevación del ST en V1-V4. ¿Cuál es la arteria probablemente ocluida?",
        image: null,
        options: ["Coronaria Derecha", "Descendente Anterior Izquierda", "Circunfleja", "Marginal Obtusa"],
        correct_idx: 1
      }
    ]
  }
...
export default function App() {
  ...
  const Button = ({ children, onClick, colorClass, textColor = "text-white", className = "" }) => (
    <button 
      onClick={onClick} 
      className={`px-4 py-2 rounded-xl font-medium transition-transform transform hover:scale-105 active:scale-95 ${colorClass} ${textColor} ${className}`}
    >
      {children}
    </button>
  );
  ...
  const Input = ({ value, onChange, placeholder, isTextArea = false }) => {
    ...
    return isTextArea ? (
      <textarea value={value} onChange={onChange} placeholder={placeholder} rows={3} className={baseClasses} />
    ) : (
      <input type="text" value={value} onChange={onChange} placeholder={placeholder} className={baseClasses} />
    );
  };
      ...
      <div className={`min-h-screen p-8 transition-colors duration-300 ${colors.bg} ${colors.fg} font-sans`}>
        ...
        <div className="max-w-5xl mx-auto space-y-8">
          <div className={`p-6 rounded-2xl shadow-sm ${colors.card_bg}`}>
            ...
            <p className={`mb-6 ${colors.text_sec}`}>
              Tarjetas pendientes: <span className="font-bold">{dueCards.length}</span> | Total: {cards.length}
            </p>
            <div className="flex space-x-4">
              ...
              <Button colorClass={colors.secondary} textColor={colors.fg} onClick={() => setCurrentView('addCard')}>
                <span className="flex items-center">➕ Crear Carta</span>
              </Button>
              <Button colorClass={colors.secondary} textColor={colors.fg} onClick={() => setCurrentView('browseCards')}>
                <span className="flex items-center">📋 Ver Todas</span>
              </Button>
            </div>
          </div>
                  ...
                  return (
                    <div key={q.id} className={`flex items-center justify-between p-4 rounded-xl border ${colors.border}`}>
                      ...
                      <div className="flex items-center space-x-2">
                        <Button colorClass={colors.accent} onClick={() => { setActiveQuiz(q); setCurrentView('takeQuiz'); }}>
                          <span className="flex items-center">▶️ Iniciar</span>
                        </Button>
                        <button onClick={() => { setActiveQuiz(q); setCurrentView('editQuiz'); }} className={`text-lg p-2 rounded-lg hover:bg-opacity-20 hover:${colors.secondary} ${colors.fg}`}>
                          ✏️
                        </button>
                        <button onClick={() => setQuizzes(quizzes.filter(quiz => quiz.id !== q.id))} className="text-lg p-2 rounded-lg text-[#FF5252] hover:bg-[#FF5252] hover:bg-opacity-10">
                          🗑️
                        </button>
                      </div>
                    </div>
  ...
  const AddCardView = () => {
    ...
    const handleImage = (e) => {
      ...
      if (file) {
        ...
        reader.onloadend = () => {
          setImageStr(reader.result); // Base64
        };
      ...
      <div className={`min-h-screen p-8 ${colors.bg} ${colors.fg} flex flex-col items-center justify-center`}>
        <div className={`w-full max-w-2xl p-8 rounded-3xl shadow-lg ${colors.card_bg}`}>
          ...
          <div className="space-y-6">
            <div>
              <label className={`block text-sm mb-2 ${colors.text_sec}`}>Pregunta</label>
              <Input isTextArea value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="Ej. ¿Síntoma principal de..." />
            </div>
            <div>
              <label className={`block text-sm mb-2 ${colors.text_sec}`}>Respuesta</label>
              <Input isTextArea value={answer} onChange={(e) => setAnswer(e.target.value)} placeholder="Ej. Dolor torácico..." />
            </div>
            ...
            <div className="flex space-x-4 pt-4">
              <Button colorClass={colors.accent} onClick={handleSave} className="flex-1">Guardar</Button>
  ...
  const QuizEditorView = () => {
    ...
    const [questions, setQuestions] = useState(activeQuiz?.questions || [
      { text: "", options: ["", "", "", ""], correct_idx: 0, image: null }
    ]);
    ...
    const handleSave = () => {
      ...
      const newQuiz = {
        ...
        repetitions: activeQuiz?.repetitions || 0,
        ef: activeQuiz?.ef || 2.5,
        interval: activeQuiz?.interval || 0
      };

      if (activeQuiz) {
        setQuizzes(quizzes.map(q => q.id === activeQuiz.id ? newQuiz : q));
      } else {
    ...
    return (
      <div className={`min-h-screen p-8 ${colors.bg} ${colors.fg}`}>
        <div className="max-w-4xl mx-auto">
          <button onClick={() => setCurrentView('dashboard')} className={`flex items-center mb-6 hover:opacity-70 ${colors.text_sec}`}>
          ...
          <div className="mb-8 space-y-4">
            ...
            <div className="flex items-center space-x-4">
              ...
              <input type="number" min="0" value={timeLimit} onChange={e => setTimeLimit(Number(e.target.value))} 
                className={`w-24 p-2 rounded-lg text-center ${colors.input_bg} ${colors.fg} outline-none`} />
            </div>
          </div>
          ...
          <div className="space-y-6 mb-8">
            {questions.map((q, qIndex) => (
              <div key={qIndex} className={`p-6 rounded-2xl shadow-sm ${colors.card_bg} relative`}>
                <button onClick={() => setQuestions(questions.filter((_, i) => i !== qIndex))} className="absolute top-4 right-4 text-red-500 hover:opacity-70">
                  🗑️
                </button>
                ...
                <div className="space-y-3">
                  {q.options.map((opt, oIndex) => (
                    <div key={oIndex} className="flex items-center space-x-3">
                      <input type="radio" name={`correct-${qIndex}`} checked={q.correct_idx === oIndex} 
                        onChange={() => {
                          const newQ = [...questions]; newQ[qIndex].correct_idx = oIndex; setQuestions(newQ);
                        }}
                        className="w-5 h-5 accent-[#4A90E2] cursor-pointer" />
                      <Input value={opt} onChange={e => {
                          const newQ = [...questions]; newQ[qIndex].options[oIndex] = e.target.value; setQuestions(newQ);
                        }} placeholder={`Opción ${['A','B','C','D'][oIndex]}`} />
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="flex justify-between items-center pb-20">
            <button onClick={() => setQuestions([...questions, { text: "", options: ["", "", "", ""], correct_idx: 0, image: null }])} 
              className={`font-bold ${colors.accent_text} hover:opacity-80 flex items-center`}>
  ...
  const ReviewCardsView = () => {
      ...
      return (
        <div className={`min-h-screen flex items-center justify-center ${colors.bg} ${colors.fg}`}>
          <div className="text-center">
            ...
            <h2 className="text-2xl font-bold">¡Repaso Terminado!</h2>
          </div>
        </div>
      );
    }
    ...
    return (
      <div className={`min-h-screen p-8 flex flex-col ${colors.bg} ${colors.fg}`}>
        <button onClick={() => setCurrentView('dashboard')} className={`self-start mb-6 hover:opacity-70 ${colors.text_sec}`}>
          <span className="mr-2">⬅️</span> Salir
        </button>
        ...
        <div className="flex-1 flex flex-col items-center justify-center max-w-3xl mx-auto w-full">
          <div className={`w-full p-10 rounded-3xl shadow-lg text-center ${colors.card_bg}`}>
            ...
            {!showAnswer ? (
              <Button colorClass={colors.accent} onClick={() => setShowAnswer(true)} className="w-64 py-3 text-lg">
                Mostrar Respuesta
              </Button>
            ) : (
  ...
  const QuizTakerView = () => {
    ...
    const handleAnswer = (selectedIndex) => {
      ...
      if (isCorrect) {
        if (phase === 'first_pass') setFirstTryScore(s => s + 1);
      } else {
        newMistakes.push(currentQ);
      }
    ...
    const handleFinishReview = (quality) => {
    ...
    return (
      <div className={`min-h-screen flex flex-col ${colors.bg} ${colors.fg}`}>
        <div className="p-6 flex justify-between items-center relative">
          ...
          {showRetryAlert && (
            <div className="absolute left-1/2 transform -translate-x-1/2 bg-[#FF5252] text-white px-6 py-2 rounded-full font-bold shadow-lg animate-pulse">
              Modo Corrección: Responde tus errores
            </div>
          )}

          <span className={colors.text_sec}>
            {phase === 'retry_pass' 
              ? `Corrección: Pregunta ${currentRoundIndex + 1} de ${activeQuestions.length}` 
              : `Pregunta ${currentRoundIndex + 1} de ${activeQuestions.length}`}
          </span>
        </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              {currentQ.options.map((opt, i) => (
                <button key={i} onClick={() => handleAnswer(i)}
                  className={`p-5 rounded-2xl text-left text-lg font-medium transition-transform transform hover:scale-[1.02] border ${colors.border} ${colors.card_bg} hover:${colors.secondary}`}>
                  {opt}
                </button>
              ))}
            </div>
    ...
    return (
      <div className={`min-h-screen p-8 ${colors.bg} ${colors.fg}`}>
        <div className="max-w-5xl mx-auto">
          <button onClick={() => setCurrentView('dashboard')} className={`flex items-center mb-6 hover:opacity-70 ${colors.text_sec}`}>
            <span className="mr-2">⬅️</span> Volver al Inicio
          </button>
          <h2 className="text-3xl font-bold mb-6">Todas las Flashcards ({cards.length})</h2>
          
          {cards.length === 0 ? (
            <p className={colors.text_sec}>Aún no has creado ninguna flashcard.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {cards.map(card => (
                <div 
                  key={card.id} 
                  className={`p-6 rounded-2xl shadow-sm border ${colors.border} ${colors.card_bg} flex flex-col cursor-pointer transition-transform hover:scale-[1.02]`} 
                  onClick={() => toggleFlip(card.id)}
                >
                  <h3 className={`text-sm font-bold tracking-widest mb-4 ${colors.accent_text}`}>
                    {flipped[card.id] ? "RESPUESTA" : "PREGUNTA"}
                  </h3>
                  {!flipped[card.id] ? (
                    <>
                      <p className="text-lg font-medium mb-4 flex-1">{card.question}</p>
                      {card.image && <img src={card.image} alt="card" className="max-h-32 rounded-lg object-cover mb-4" />}
                    </>
                  ) : (
                    <p className={`text-lg mb-4 flex-1 ${colors.text_sec}`}>{card.answer}</p>
                  )}
                  <p className={`text-xs mt-auto pt-4 border-t ${colors.border} ${colors.text_sec} text-center`}>
                    Click para voltear
                  </p>
                </div>
              ))}
